...
async function main() {
  ...
  console.log("Transferring to multiple addresses with separate transactions:");
  for (const receiver of receivers) {
    console.log(`- ${receiver.address}: ${receiver.amount} CKB`);
  }
  
  // Process each receiver separately
  for (const receiver of receivers) {
    console.log(`Processing transaction for ${receiver.address} (${receiver.amount} CKB)...`);
    
    // Parse the receiver script from address
    const { script: lock } = await ccc.Address.fromString(
      receiver.address,
      signer.client,
    );
    
    // Create a transaction for this specific receiver
    const tx = ccc.Transaction.from({
      outputs: [
        { 
          lock,
          capacity: ccc.fixedPointFrom(receiver.amount)
        }
      ]
    });
...
// https://testnet.explorer.nervos.org/block/16564754
